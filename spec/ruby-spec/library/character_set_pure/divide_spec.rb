...
describe "CharacterSet::Pure#divide" do
  it "divides self into a set of subsets based on the blocks return values" do
    set = CharacterSet::Pure[1, 2, 3, 4, 5].divide(&:odd?)
    set.map { |x| x.to_a }.to_a.sort.should == [[1, 3, 5], [2, 4]]
  end

  it "yields each Object in self in sorted order" do
    ret = []
    CharacterSet::Pure[1, 2, 3, 4, 5].divide { |x| ret << x }
  ...
  #   lambda { CharacterSet::Pure[1].divide }.should raise_error(LocalJumpError)
  # end
end

describe "CharacterSet::Pure#divide when passed a block with an arity of 2" do
  it "divides self into a set of subsets based on the blocks return values" do
    set = CharacterSet::Pure[1, 3, 4, 6, 9, 10, 11].divide { |x, y| (x - y).abs == 1 }
    set.map { |x| x.to_a }.to_a.sort.should == [[1], [3, 4], [6], [9, 10, 11]]
  end

  it "yields each two Objects to the block" do
    ret = []
    CharacterSet::Pure[1, 2].divide { |x, y| ret << [x, y] }
